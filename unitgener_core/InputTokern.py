# __author__     = "Sachith Senarathne"
# __copyright__  = "Copyright 2017, The UnitGener Project"
# __license__    = "MIT"
# __version__    = "0.1"
# __maintainer__ = "Sachith Senarathne"
# __email__      = "sachith.senarathnes@gmail.com"
...
def read_process_file():
    ...
    for line in content:
        if line != '/n':
            js_tokens = []
            string_tokens = pre_process_tokens(line.split())
            count = 0
            ...
            variable_token_result = process_decl_tokens(js_tokens)
            format_result_tokens = conditional_token_processor(variable_token_result)
            result_tokens = literal_token_processor(format_result_tokens)
            return_tokens = statement_token_builder(result_tokens)
            ...
            # for function_tokens in _result_list:
            #     print function_tokens


def pre_process_tokens(str_tokens):
    _return_tokens = []
...
def conditional_token_processor(js_tokens):
    _processed_tokens = []
...
def literal_token_processor(format_result_tokens):
    _lit_tokens = []
...
def process_decl_tokens(js_tokens):
    _decl_tokn_list = []
    for idx, dec_token in enumerate(js_tokens):
        if dec_token == 'var':
            temp_tkn = dec_token + ' ' + js_tokens[idx + 1]
            js_tokens.pop(idx + 1)
            _decl_tokn_list.append(temp_tkn)
        else:
            if not dec_token == '':
                _decl_tokn_list.append(dec_token)
    return _decl_tokn_list


def arr_token_processor(variable_token_result):
    _arr_results = []
...
def check_for_special_character(token, _char):
    _is_available = False
